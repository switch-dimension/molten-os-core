...
Before asking anything, check the working directory (and obvious sibling locations like `.docs/`, `.planning/`, or the project root) for a `brand.md` and/or a `design.md`. These are produced by companion skills and, when present, are the source of truth — don't re-ask what they already answer.
